...
export const StorageService = {
  ...
  recordEggSalePOS: (params: {
    ...
    if (metodeBayar === 'TEMPO') {
      ...
      // 2. Jurnal Piutang (Debit Piutang 103, Kredit Pendapatan 401)
      StorageService.addTransaksiKeuangan({
        tanggal,
        deskripsi: `[TEMPO/PIUTANG] ${deskripsi}`,
        totalNominal,
        tipeTransaksi: 'PENDAPATAN',
        kategoriPendapatan: kategori,
        items: [
          { akunId: '103', debit: totalNominal, kredit: 0 }, // Piutang Usaha
          { akunId: '401', debit: 0, kredit: totalNominal }, // Pendapatan Telur
        ],
        createdBy: 'Kasir POS',
      });
    } else {
      // Tunai / Transfer langsung Kas Masuk
      const akunKasId = metodeBayar === 'TRANSFER' ? '102' : '101'; // 102: Bank, 101: Kas
      StorageService.addTransaksiKeuangan({
        tanggal,
        deskripsi,
        totalNominal,
        ...
        kategoriPendapatan: kategori,
        items: [
          { akunId: akunKasId, debit: totalNominal, kredit: 0 },
          { akunId: '401', debit: 0, kredit: totalNominal },
        ],
        createdBy: 'Kasir POS',
      });
    }
  ...
  // Pay Debt or Receivable Installment (Cicilan / Pelunasan)
  payHutangPiutang: (params: {
    hpId: string;
    nominalBayar: number;
    tanggal: string;
    akunKasId?: string; // 101 (Kas) / 102 (Bank)
    catatan?: string;
  }) => {
    const { hpId, nominalBayar, tanggal, akunKasId = '101', catatan } = params;
    const hpList = StorageService.getHutangPiutang();
    const targetIndex = hpList.findIndex((h) => h.id === hpId);

    if (targetIndex === -1) throw new Error('Data hutang/piutang tidak ditemukan');

    const item = hpList[targetIndex];
    const newSisa = Math.max(0, item.sisaNominal - nominalBayar);
    const newStatus = newSisa === 0 ? 'LUNAS' : 'BELUM_LUNAS';

    const updatedItem: HutangPiutang = {
      ...item,
      sisaNominal: newSisa,
      status: newStatus,
    };

    const updatedList = [...hpList];
    updatedList[targetIndex] = updatedItem;
    StorageService.saveHutangPiutang(updatedList);

    // Record Financial Transaction
    if (item.jenis === 'PIUTANG') {
      // Receive payment -> Kas Masuk (Debit Kas 101, Kredit Piutang 103)
      StorageService.addTransaksiKeuangan({
        tanggal,
        deskripsi: `Penerimaan Pelunasan/Cicilan Piutang: ${item.namaKontak} (${item.deskripsi})${catatan ? ` - ${catatan}` : ''}`,
        totalNominal: nominalBayar,
        tipeTransaksi: 'PENDAPATAN',
        kategoriPendapatan: 'LAINNYA',
        items: [
          { akunId: akunKasId, debit: nominalBayar, kredit: 0 },
          { akunId: '103', debit: 0, kredit: nominalBayar },
        ],
        createdBy: 'Owner',
      });
    } else {
      // Pay Debt -> Kas Keluar (Debit Hutang 201, Kredit Kas 101)
      StorageService.addTransaksiKeuangan({
        tanggal,
        deskripsi: `Pembayaran Cicilan/Pelunasan Hutang: ${item.namaKontak} (${item.deskripsi})${catatan ? ` - ${catatan}` : ''}`,
        totalNominal: nominalBayar,
        tipeTransaksi: 'PENGELUARAN',
        kategoriPengeluaran: 'OPERASIONAL_KANDANG',
        items: [
          { akunId: '201', debit: nominalBayar, kredit: 0 },
          { akunId: akunKasId, debit: 0, kredit: nominalBayar },
        ],
        createdBy: 'Owner',
      });
    }
  ...
  importAllDataJSON: (jsonData: any) => {
    if (!jsonData || typeof jsonData !== 'object') {
      throw new Error('Format file JSON tidak valid');
    }

    if (jsonData.kandang) StorageService.saveKandang(jsonData.kandang);
    if (jsonData.populasi) StorageService.savePopulasi(jsonData.populasi);
    if (jsonData.pakan) StorageService.savePakan(jsonData.pakan);
    if (jsonData.pencatatan_harian) StorageService.savePencatatanHarian(jsonData.pencatatan_harian);
    if (jsonData.transaksi_keuangan) StorageService.saveTransaksi(jsonData.transaksi_keuangan);
    if (jsonData.aset_tetap) StorageService.saveAset(jsonData.aset_tetap);
    if (jsonData.hutang_piutang) StorageService.saveHutangPiutang(jsonData.hutang_piutang);
    if (jsonData.kode_akun) setStoredData('kode_akun', jsonData.kode_akun);
  },
};
